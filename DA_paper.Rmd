...
qplt2 <- ggplot(QQ1, aes(x=log(population))) + geom_histogram()
...
p1<- ggpubr::ggarrange(qplt1, qplt4, qplt2, qplt3, nrow = 2, ncol = 2)
p1
```
...
\multirow{3}{*}{Normal-gamma(Constrained)}
...
\multirow{3}{*}{T-betaprime(Flexible)}
...
```{r long, out.width='60%', echo=FALSE, fig.cap="Density plots for number of occupied tables in the restraunt process with subtitles representing different values in alpha."}
...
one_qq <- function(config, fit, strg, sam) {
  ...
  qplt <- ggplot() + geom_line(data = qq, aes(x = pop, y = pred), color = "#999999") + theme(legend.position = "none")  + theme(axis.title.x=element_blank(),
  ...
  for (j in c(10:24)) {
    qq <- data.frame(pred = sort((fit[[config]]$fit[[sam]]$pred[j,])),
                   pop = sort((fit[[config]]$popdata[[sam]])))
    qplt <- qplt + geom_line(data = qq, aes(x = pop, y = pred), color = "#999999") + theme(legend.position = "none") + theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        axis.title.y=element_blank(),
        axis.text.y=element_blank(),
  ...
  qplt <- qplt + geom_abline(aes(slope = 1, intercept = 0,colour = "red"))+
    ggtitle(strg[config]) + theme(legend.position = "none") + theme(plot.title = element_text(size=8, color = "blue")) + theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        axis.title.y=element_blank(),
        axis.text.y=element_blank(),
...
qq <- ggpubr::ggarrange(
  ...
  nrow = 3, ncol = 4, legend = NULL)
ggpubr::annotate_figure(qq,
                bottom = ggpubr::text_grob("population"),
                left = ggpubr::text_grob("prediction")
)
```
...
\begin{tabular}{ |c|c|c|c| } 
 \hline
 parameter (within parenthesis are those in TB prior) & NG & TB & kde \\ 
 \hline
 $v_0 = 2.13, ss_0=0.6,g_1=1,g_2=4,c=3,d=0.35,(df=3,v_1=1.5)$ & 0.047 & 0.044 & 0.052\\ 
 $v_0 = 2.13, ss_0=0.6,g_1=1,g_2=4,c=11,d=1,(df=3,v_1=1.5)$ & 0.045 & 0.046 & 0.052\\
 $v_0 = 2.13, ss_0=0.6,g_1=0.5,g_2=2,c=3,d=0.35,(df=3,v_1=1.5)$ & 0.049 & 0.049 & 0.052\\
 $v_0 = 2.13, ss_0=0.6,g_1=0.5,g_2=2,c=11,d=1,(df=3,v_1=1.5)$ & 0.048 & 0.046 & 0.052\\
 $v_0 = 4, ss_0=1.75,g_1=1,g_2=4,c=3,d=0.35,(df=3,v_1=1.5)$ & 0.048 & 0.047 & 0.052\\
 $v_0 = 4, ss_0=1.75,g_1=1,g_2=4,c=11,d=1,(df=3,v_1=1.5)$ & 0.050 & 0.047 & 0.052\\
 $v_0 = 4, ss_0=1.75,g_1=0.5,g_2=2,c=3,d=0.35,(df=3,v_1=1.5)$ & 0.050 & 0.047 & 0.052\\
 $v_0 = 4, ss_0=1.75,g_1=0.5,g_2=2,c=11,d=1,(df=3,v_1=1.5)$ & 0.051 & 0.050 & 0.052\\
 $v_0 = 10, ss_0=6.2,g_1=1,g_2=4,c=3,d=0.35,(df=3,v_1=1.5)$ & 0.049 & 0.044 & 0.052\\
 $v_0 = 10, ss_0=6.2,g_1=1,g_2=4,c=11,d=1,(df=3,v_1=1.5)$ & 0.050 & 0.044 & 0.052\\
 $v_0 = 10, ss_0=6.2,g_1=0.5,g_2=2,c=3,d=0.35,(df=3,v_1=1.5)$ & 0.049 & 0.044 & 0.052\\
 $v_0 = 10, ss_0=6.2,g_1=0.5,g_2=2,c=11,d=1,(df=3,v_1=1.5)$ & 0.050 & 0.046 & 0.052\\ 
 \hline
\end{tabular}
\caption{KS test statistic table; A smaller test statistic value means better fit. NG stands for normal-gamma prior and TB stands for t-betaprime prior.}
\end{table}


```{r qplt_flex_sam1, out.width='70%', echo = FALSE, longtable = TRUE, warning=FALSE, cache=TRUE, fig.cap="Two sample QQ plot for different configurations with flexible model (fitted based on random sample 1); X-axis represents hold-out population data; Y-axis represents predictions; Subtitles represent different configurations, which can be referenced in appendix C."}
...
one_qq <- function(config, fit, strg, sam) {
  ...
  qplt <- qplt + geom_abline(aes(slope = 1, intercept = 0,colour = "red")) + theme(legend.position = "none") + theme(plot.title = element_text(size=8, color = "blue")) + theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        axis.title.y=element_blank(),
        axis.text.y=element_blank(),
...
cmodel_conf <- as.character(c(1:12))
one_bxplt1 <- function(config, nppred, fmmp, bsp, cltp, strg) {
  ...
  p1 <- ggplot(df1, aes(x = method, y = value)) + geom_boxplot() + ggtitle(strg[config]) +
    theme(plot.title = element_text(size=8, color = "blue")) + theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.title.y=element_blank(),
        axis.text.y=element_blank())
  return(p1)
...
one_bxplt2 <- function(config, nppred, fmmp, bsp, cltp, strg) {
  df1 <- data.frame(method = rep(NA, 6*500), value = rep(NA, 6*500))
  df1$method[1:500] <- "T"
  df1$value[1:500] <- nppred[[config]]$true*8983
  df1$method[501:1000] <- "N"
...
one_bxplt4 <- function(config, nppred, fmmp, bsp, cltp, strg) {
  df1 <- data.frame(method = rep(NA, 6*500), value = rep(NA, 6*500))
  df1$method[1:500] <- "T"
  df1$value[1:500] <- nppred[[config]]$true*8983
  df1$method[501:1000] <- "N"
...
figure <- ggpubr::ggarrange(
  one_bxplt1(1,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt2(2,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt2(3,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt2(4,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt1(5,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt2(6,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt2(7,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt2(8,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt4(9,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt4(10,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt4(11,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  one_bxplt4(12,cons_mean, fmm_mean, bs_mean, clt_mean, cmodel_conf),
  nrow = 3, ncol = 4)
ggpubr::annotate_figure(figure,
                bottom = ggpubr::text_grob("method"))
```
...
load("~/Projects/DAfinal/fmm_mean.Rdata")
cmodel_conf <- as.character(c(1,7,13,19,25,31,37,43,49,55,61,67))
one_bxplt1 <- function(config, nppred, fmmp, bsp, cltp) {
...
one_bxplt2 <- function(config, nppred, fmmp, bsp, cltp) {
  df1 <- data.frame(method = rep(NA, 500*6), value = rep(NA, 6*500))
  df1$method[1:500] <- "T"
  df1$value[1:500] <- nppred[[config]]$true*8983
  df1$method[501:1000] <- "N"
...
p1 <- one_bxplt1(1, flex_mean_p1, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[1])
p2 <- one_bxplt1(1, flex_mean_p2, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[2])
p3 <- one_bxplt1(1, flex_mean_p3, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[3])
p4 <- one_bxplt1(1, flex_mean_p4, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[4])
p5 <- one_bxplt1(1, flex_mean_p5, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[5])
p6 <- one_bxplt1(1, flex_mean_p6, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[6])
p7 <- one_bxplt1(1, flex_mean_p7, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[7])
p8 <- one_bxplt1(1, flex_mean_p8, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[8])
p9 <- one_bxplt2(1, flex_mean_p9, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[9])
p10 <- one_bxplt2(1, flex_mean_p10, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[10])
p11 <- one_bxplt2(1, flex_mean_p11, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[11])
p12 <- one_bxplt2(1, flex_mean_p12, fmm_mean, bs_mean, clt_mean) + ggtitle(cmodel_conf[12])
qq <- ggpubr::ggarrange(
  p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,
  nrow = 3, ncol = 4)
ggpubr::annotate_figure(qq,
...
$$R = \sqrt{\dfrac{\sum_{i =1}^{n}(log(pred^{(i)}) - log(true^{(i)}))^2}{n}}.$$
...
\begin{tabular}{ |c|c|c| } 
 \hline
 parameter (within parenthesis are those in TB prior) & NG & TB \\ 
 \hline
 ...
 \hline
\end{tabular}
\caption{RMSLE table for bayesian mixture model.}
\end{table}
